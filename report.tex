...
\usepackage{multirow}
\usepackage{xcolor}
% microtype removed - font expansion incompatible with available LaTeX fonts on this machine
\usepackage{hyperref}
\usepackage{titlesec}
\usepackage{authblk}
\usepackage{caption}
...
\subsection{CLIFF, Democritus, BASKET, FunctorFlow~\cite{cliff,democritus_repo,basket,functorflow}}

Mahadevan's research toolchain underpins our project. \textbf{Democritus}~\cite{democritus_repo} extracts causal triples from documents via auto-proposed root topics, a topic graph, causal questions/statements, and a final triple-extraction stage; its geometric back-end uses the Geometric Transformer (GT) and Diagrammatic Backpropagation (DB) to build a $2$D/$3$D causal manifold. \textbf{CLIFF}~\cite{cliff} is an agentic chatbot built on FunctorFlow with built-in information-gain tracking. \textbf{BASKET}~\cite{basket} builds agentic systems from textual documents using Kan-Extension Transformers (KETs~\cite{ket}); its prototype implements a ``PLAN-KET block encoder $\to$ corruption $\to$ denoising / repair'' pipeline. \textbf{FunctorFlow}~\cite{functorflow} is the categorical-ML framework on which the above projects build. Our \texttt{kan\_transfer/} loads the triples produced by Democritus directly from \texttt{relational\_triples.jsonl} and uses \texttt{sentence-transformers} (\texttt{all-MiniLM-L6-v2}, $384$-d) for query/topic embeddings; we do not import any FunctorFlow code, and the \texttt{cliff\_integration/kan\_transfer\_agentic.py} module is a router stub for future integration rather than an active dependency. On top of these inputs we layer the two Kan-extension implementations, the soft Kan, and the seven-dimension evaluation suite.
...
\begin{tabular}{lrrrl}
\toprule
Corpus & Docs & Triples & Topics & Role \\
\midrule
Economic (Beige Book + FOMC + NBER + FEDS + BIS) & 93  & 313 & 92 & GT target \\
Legal (SEC + CFPB + FTC)                         & 31  &  43 & 12 & Source A \\
Medical (PubMed full-text)                       & 101 &  47 & 12 & Source B \\
\bottomrule
\end{tabular}
\end{center}

The Democritus record is $\{\text{topic}, \text{path}, \text{question}, \text{statement}, \text{subj}, \text{rel}, \text{obj}, \text{domain}\}$; the Kan-extension code uses the subset $(\text{subj}, \text{rel}, \text{obj}, \text{topic})$. The realised relation set is $\{\text{causes}, \text{increases}, \text{reduces}, \text{influences}, \text{affects}, \text{leads\_to}\}$ for economic and legal, and $\{\text{causes}, \text{increases}, \text{reduces}, \text{influences}, \text{leads\_to}\}$ for medical (which never emitted \texttt{affects}). The query split uses 20 held-out economic queries.
...
\texttt{kan/baseline.py} reads the source functor's raw triple list (\texttt{source\_functor.triples}), retrieves the top-20 triples by direct query-to-triple cosine similarity, and returns them as a graph. No functor structure, no categorical composition, no Kan extension.

\subsection{Seven evaluation dimensions}

For each of $\{\text{left\_kan}, \text{right\_kan}, \text{naive\_rag}\}$ and each of $20$ test queries we compute all seven dimensions below; the soft Kan variants $\{\text{soft\_left\_kan}, \text{soft\_right\_kan}\}$ are evaluated only on Semantic Soft-F1 and Motif in the current pipeline (extending them through the remaining four evaluators is a half-day implementation task, \S\ref{sec:cw}).
...
\caption{Mean edge-F1 of left vs.\ right Kan as a function of domain proximity. Exact-match F1 is $0$ in both cases (see \S\ref{sec:zerof1}); the empirical content lies in edge counts (Table~\ref{tab:edges}). Domain proximities: medical${\to}$economic $= 0.0354$; legal${\to}$economic $= 0.4802$.}
\label{fig:crossover}
\end{figure}

\paragraph{Edge-count asymmetry.} Across all 20 legal${\to}$economic queries, left Kan produces an average of \textbf{6.75 edges/graph} (max 18); right Kan \textbf{0.40 edges/graph} (max 4). This ${\sim}17{:}1$ ratio is the qualitative pattern predicted by the coend / end duality. For medical${\to}$economic --- where source and target share virtually no semantic ground (proximity $0.0354$) --- both hard-Kan variants collapse to the empty graph, also as predicted. The soft-Kan variants partially break this collapse (\S\ref{sec:softkan}).
...
left\_kan  & 6.75 & 18 \\
right\_kan & 0.40 &  4 \\
soft\_left\_kan ($\alpha{=}0.5$) & \textbf{35.10} & 38 \\
soft\_right\_kan                  & 0.00  &  0 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering\small
\caption{Legal${\to}$Economic (proximity $0.4802$, populated regime). \texttt{n/a} = pipeline not yet wired for soft variants on these evaluators (\S\ref{sec:cw}).}\label{tab:legal}
...
soft\_left\_kan & 0.027           & 0.135           & n/a            & n/a            & n/a            & n/a            \\
soft\_right\_kan& 0.000           & 0.000           & n/a            & n/a            & n/a            & n/a            \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering\small
\caption{Medical${\to}$Economic (proximity $0.0354$, degenerate regime). Notable: medical \texttt{soft\_left\_kan} motif $0.354$ is the highest motif score of any (method, source) pair in this study, despite semantic soft-F1 remaining $0$.}\label{tab:medical}
\begin{tabular}{lrrrrrr}
\toprule
Method & SoftF1 & Motif & Homol. & Univ. & BackT & Coher. \\
\midrule
...
soft\_left\_kan & 0.000 & \textbf{0.354} & n/a   & n/a   & n/a   & n/a   \\
soft\_right\_kan& 0.000 & 0.050          & n/a   & n/a   & n/a   & n/a   \\
\bottomrule
\end{tabular}
\end{table}
...
Across all $20$ unique hyperparameter combinations ($3{\times}3$ grid in $(k, \tau)$ for both methods at $\mathrm{consensus\_frac}{=}0.60$, plus $2$ additional consensus values for right Kan at $k{=}10, \tau{=}0.25$) and $20$ queries, edge-F1 is identically $0.0$. Predicted edges live in source-domain vocabulary (e.g., \texttt{"sec enforcement"}, \texttt{"clinical trial"}) while ground truth lives in target vocabulary (e.g., \texttt{"open market operations"}); string equality is mathematically impossible. This is the \emph{measurement gap} motivating the semantic and structural evaluators.
...
\caption{Causal-graph comparison for legal${\to}$economic query ``Roles and responsibilities of individual board members''. Left to right: Ground Truth, Left Kan, Right Kan, Naive RAG. Green/red/gray = TP/FP/FN. Exact-match F1 is $0$ everywhere but \emph{graph shape} differs meaningfully --- exactly what the motif / homology evaluators are designed to detect.}
\label{fig:perquery}
\end{figure}

\subsection{The Soft-Kan / RN-derivative experiment}\label{sec:softkan}

\begin{table}[h]
\centering\small
\caption{Soft Kan ($\alpha{=}0.5$) vs. hard Kan, both source domains.}\label{tab:softkan}
\begin{tabular}{llrrr}
\toprule
Source & Method & avg edges & $\rho$SoftF1 & Motif \\
\midrule
\multirow{4}{*}{Legal}   & left\_kan       &  6.75 & 0.029 & 0.135 \\
                         & soft\_left\_kan & \textbf{35.10} & 0.027 & 0.135 \\
                         & right\_kan      &  0.40 & 0.003 & 0.100 \\
                         & soft\_right\_kan&  0.00 & 0.000 & 0.000 \\
\midrule
\multirow{4}{*}{Medical} & left\_kan       &  0.00 & 0.000 & 0.000 \\
                         & soft\_left\_kan & \textbf{19.65} & 0.000 & \textbf{0.354} \\
                         & right\_kan      &  0.00 & 0.000 & 0.000 \\
                         & soft\_right\_kan&  0.40 & 0.000 & 0.050 \\
\bottomrule
\end{tabular}
\end{table}
...
(1)~\textbf{On legal, soft-left-Kan produces ${\sim}5{\times}$ more transferred edges than hard left-Kan} ($35.10$ vs.\ $6.75$), confirming that $\rho$-reweighting upweights rare cross-domain topics. Semantic soft-F1 is essentially unchanged ($0.027$ vs.\ $0.029$) and the motif fingerprint is \emph{identical} ($0.135$ in both): the added edges come from topics with similar relational structure to the original top-$k$, so the soft Kan densifies the graph without altering its structural signature. RN-reweighting at the topic-selection stage does not close the vocabulary gap.

(2)~\textbf{On medical, soft-Kan partially escapes the empty-graph collapse.} Where hard left-Kan produces $0$ edges in $20/20$ medical queries, soft-left-Kan produces $19.65$ edges on average --- and recovers a motif score of $\mathbf{0.354}$, the highest motif score across all Kan-based methods (hard or soft) on either source. (Naive RAG scores higher in absolute terms --- $0.447$ on medical and $0.435$ on legal --- but this is the surface artefact discussed in \S\ref{sec:5-1}: RAG rates the source graph against a graph derived from the source graph, so its motif score is essentially measuring self-similarity.) Semantic soft-F1 for soft-left-Kan medical remains $0$ (vocabulary still does not overlap), but the structural fingerprint is recovered. The RN-derivative is therefore doing the work it was theoretically designed to do, in the \emph{motif/structure space} --- not in the surface-semantic space.

(3)~\textbf{The colimit/limit asymmetry persists in the soft setting.} Soft-right-Kan stays empty on legal ($0.00$) and near-empty on medical ($0.40$); soft-left-Kan densifies on both. This mirrors the \S\ref{sec:zerof1} hard-Kan asymmetry exactly: free union scales with RN reweighting, strict intersection does not.

\section{Discussion}

\subsection{What the seven dimensions actually say}\label{sec:5-1}

\paragraph{(1) Left Kan strictly dominates Right Kan on every comparable dimension.} Of the seven evaluators, six compare \texttt{left\_kan} vs.\ \texttt{right\_kan} directly (the seventh, $\rho$SoftF1, is exclusively for soft variants). Left Kan beats Right Kan on all six: SoftF1 $0.029{>}0.003$, Motif $0.135{>}0.100$, Homology $0.592{>}0.095$, Universality $0.766{>}0.743$, BackT $0.339{>}0.072$, Coherence $0.500{>}0.100$. This is the cleanest empirical confirmation of the coend / end asymmetry we are aware of.

\paragraph{(2) Naive RAG ``wins'' most absolute numbers --- but it does not actually transfer.} RAG dumps the top-$k$ source triples verbatim. Its homology score $0.997$ is because two graphs built by the same Democritus-style extractor share structural feature statistics (degree distribution, relation-type histogram, density) regardless of content domain --- the homology evaluator strips entity labels. Its coherence $0.984$ is the LLM-judge rating the source-domain triples as semantically coherent on their own terms (the prompt asks ``is this a valid causal claim?'', not ``is this economic?''). Its back-translation $0.703$ is a near-trivial identity round-trip. The professor's instinct --- ``\emph{RAG really should not work that well in transferring causality because it is not aiming to capture causal structure}'' --- is empirically borne out. The number that distinguishes cross-domain Kan transfer from strict-intersection Kan transfer is Coherence: left\_kan $0.500$ vs.\ right\_kan $0.100$, a $5{\times}$ margin. Aggregate: of the $10$ LLM-judged left\_kan queries, $5$ received a perfect raw $5/5$ ($\text{coherence}{=}1.0$) and $5$ had no predictions to judge ($n\_judged{=}0$); the non-empty queries are unanimously rated as valid economic reasoning --- Gadamer's fusion-of-horizons working as intended on the queries where the source category has anything to map.

\paragraph{(3) Proximity-predicts-transfer (H2) is sharply confirmed.} At proximity $0.0354$, six of seven dimensions are zero for both \emph{hard} Kan variants (Universality $0.715$ is an empty-graph artifact). At proximity $0.4802$, transferred edges populate every dimension. The transition is monotonic and clean for hard Kan; the soft Kan variants partially break it in the motif dimension on medical (\S\ref{sec:softkan}).

\subsection{The zero exact-match F1 finding is theoretical, not a failure}

Across all $20$ unique hyperparameter combinations and $20$ queries, exact-match edge-F1 is $0.000$. This is a mathematical guarantee of cross-domain transfer, not a method failure. Its narrative role is to \emph{empirically motivate the RN layer}: closing the gap requires $\rho(c, d)$ not just as a \emph{selection weight} (which we implement) but as an \emph{entity-translation operator} (which we do not). This is the textbook's three-layer Sheaf--Kan--RN framework with the third layer still open.

\subsection{The bottleneck is vocabulary grounding}

The soft-Kan experiment (\S\ref{sec:softkan}) is dispositive. On legal, $\alpha{=}0.5$ yields $5{\times}$ more edges ($35.10$ vs.\ $6.75$) but essentially identical semantic F1 ($0.027$ vs.\ $0.029$) and \emph{identical} motif fingerprint ($0.135$ in both). On medical, soft Kan goes from $0$ to $19.65$ edges/graph and recovers a motif score of $0.354$, yet semantic F1 stays at $0$. The pattern is the same in both regimes: the RN-derivative reweighting changes the predicted graph in \emph{structure-preserving} ways without ever bridging the source-target vocabulary. Adding more candidate triples does not help; the missing operator is one that \emph{renames} \texttt{"enforcement action"} ${\to}$ \texttt{"regulatory sanction"} while preserving causal structure. This is precisely the RN entity-translation operator.
...
\section{Current Work and Future Directions}\label{sec:cw}
...
(1) Extend the soft-Kan evaluation to the full $6$-metric pass: the soft variants are currently routed through Semantic Soft-F1 and Motif only (\texttt{run\_novel\_evaluation.py} lines $310$ and $319$); Homology, Universality, Back-Translation, and Coherence are not yet plumbed through for \texttt{soft\_method\_preds}, and the motif output lands in a \texttt{soft\_kan/} subdirectory rather than being merged into \texttt{master\_summary.json}'s top-level \texttt{motif} block. The line-$318$ comment promises ``motif/homology'' but only motif is actually called. (2) Wire \texttt{cliff\_integration/kan\_transfer\_agentic.py} ($166$-line stub) into CLIFF's agentic-router. (3) Re-run with the Anthropic Claude backend; the \texttt{extraction/anthropic\_client.py} skeleton ($125$ lines, implements an LLMClient-protocol \texttt{AnthropicChatClient}) is in place. This would partially satisfy the multi-LLM ``functorial equivalence'' question.

\section{Conclusion}

We built a complete, reproducible empirical test of Mahadevan's \emph{Kan-Do-Calculus}. The duality between coend (left Kan) and end (right Kan) is not a formal nicety: it predicts, and our experiments confirm, an asymmetric structural transfer where left Kan dominates right Kan on all six comparable evaluation dimensions at legal${\to}$economic proximity $0.4802$, while both hard-Kan variants collapse on medical${\to}$economic at proximity $0.0354$. The seven-dimension evaluation framework discriminates honest cross-domain transfer (left Kan, coherence $0.500$) from surface-preservation artefacts (naive RAG, homology $0.997$) and locates the remaining bottleneck: the RN entity-translation operator. The soft Kan we built recovers non-trivial structural transfer (motif $0.354$) on medical${\to}$economic, an empirical signature that the RN-derivative works as predicted in the structure space. Closing the residual gap with a unified causal manifold trained by Diagrammatic Backpropagation on a pooled cross-domain corpus is the natural next experiment and is the one the textbook's three-layer framework explicitly calls for.

\section*{Acknowledgements}

I thank Prof.\ Sridhar Mahadevan for the \emph{Categories for AGI} textbook, for the CLIFF / Democritus / BASKET / FunctorFlow toolchain that made this project feasible in a roughly two-week implementation window, for the project-conversation suggestion of the unified causal manifold as the natural next step (\S\ref{sec:cw}), and for the compositional-causality diagnostic for RAG.
...
\item \textbf{\texttt{sentence-transformers}} (open-source library from sbert.net / UKP Lab, \emph{not} an OpenAI product) supplied the embedding model \texttt{all-MiniLM-L6-v2}, which originates from Microsoft Research's MiniLM work.
\end{itemize}
...
\item \textbf{GitHub repository:} \url{https://github.com/JineshwarNariani/kan-extension-transfer} (this report and the full pipeline).
\item \textbf{Codebase root:} \texttt{kan\_transfer/}
\item \textbf{Headline results:} \texttt{results/tables/novel\_evaluation/master\_summary.json}
\item \textbf{Figures:} \texttt{results/figures/} ($10$ plots; the report cites four).
\item \textbf{UNITY job:} \texttt{unity\_56498824} --- $30$ min wall time, $128$ GB RAM, $16$ CPUs, CPU partition (per \texttt{unity\_job.sh}: \texttt{--mem=128G --cpus-per-task=16 --partition=cpu --time=12:00:00}; the job completed in $30$ min, well under the $12$ h cap).
\item \textbf{Extraction stats:} \texttt{results/extraction/\{economic,legal,medical\}/extraction\_stats.json}
\item \textbf{Predictions (pickled):} \texttt{results/kan\_predictions/predictions\_\{legal,medical\}.pkl}
\end{itemize}

\section{The 20 held-out economic queries}

\texttt{results/query\_split/test\_queries.json} (selected): ``Roles and responsibilities of individual board members''; ``Federal Reserve Annual Reports and Financial Disclosures''; ``Unconventional monetary policy tools during economic crises''; ``Open market operations and asset purchase programs''; ``Cross-border financial contagion and central bank coordination frameworks''; ``The Taylor Rule and systematic policy frameworks''; ``Beige Book Regional Economic Condition Reports'';\ldots\ ($20$ total).
